...
def question_keyboard(buttons: list[str] | None = None, allow_multiple: bool = False) -> ReplyKeyboardMarkup:
    ...
    placeholder = (
        "Можно выбрать несколько вариантов или написать свой ответ"
        if allow_multiple
        else "Напиши ответ или нажми кнопку"
    )
    return ReplyKeyboardMarkup(
        keyboard=rows,
        resize_keyboard=True,
...
def unsupported_topic_message(data: dict[str, Any]) -> str | None:
    ...
    answer = (
        ...
        or "Пока я не умею собрать хороший трек по этой теме 😔 Но мы можем выбрать одно из доступных направлений."
    )
    if available:
        answer = f"{answer}\n\nМожно начать с этих направлений:\n" + "\n".join(
            f"• {area}" for area in available
        )
    return answer
...
async def finish_roadmap(message: Message, telegram_id: int, state: TrackSession) -> None:
    await message.answer(
        "✅ Профиль собран. Сейчас соберу персональный roadmap и подберу материалы...",
